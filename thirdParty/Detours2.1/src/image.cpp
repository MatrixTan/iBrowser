...
public:                                                 // Manipulation Functions
    ...
    BOOL                    EditAddImports(PVOID pContext,
                                        PF_DETOUR_BINARY_BYWAY_CALLBACK pfBywayCallback,
                                        PF_DETOUR_BINARY_FILE_CALLBACK pfFileCallback,
                                        PF_DETOUR_BINARY_SYMBOL_BYWAY_CALLBACK pfByWaySymbolCallback,
                                        PF_DETOUR_BINARY_SYMBOL_CALLBACK pfSymbolCallback,
                                        PF_DETOUR_BINARY_COMMIT_CALLBACK pfCommitCallback);
protected:
    ...
    CImageImportFile *      NewBywayWithSymbol(__in_z PCHAR pszName, int n);
...
CImageImportFile * CImage::NewBywayWithSymbol(__in_z PCHAR pszName, int nSymbolCount)
{
    CImageImportFile *pImportFile = new CImageImportFile;
    if (pImportFile == NULL) {
        SetLastError(ERROR_OUTOFMEMORY);
    ...
    pImportFile->m_rvaFirstThunk = 0;
    pImportFile->m_nForwarderChain = (UINT)0;
	pImportFile->m_pImportNames =new CImageImportName[nSymbolCount];
	pImportFile->m_nImportNames = nSymbolCount;
...
BOOL CImage::EditAddImports(PVOID pContext,
							PF_DETOUR_BINARY_BYWAY_CALLBACK pfBywayCallback,
							PF_DETOUR_BINARY_FILE_CALLBACK pfFileCallback,
							PF_DETOUR_BINARY_SYMBOL_BYWAY_CALLBACK pfByWaySymbolCallback,
							PF_DETOUR_BINARY_SYMBOL_CALLBACK pfSymbolCallback,
							PF_DETOUR_BINARY_COMMIT_CALLBACK pfCommitCallback)
{
    ...
    while ((pImportFile = *ppLastFile) != NULL) {
        ...
        if (pfBywayCallback) {
            ...
            if (pszFile) {
                // Insert a new Byway.
                CImageImportFile *pByway = NewBywayWithSymbol(pszFile, 50);
                if (pByway == NULL) {
                    return FALSE;
                }
				
			if (pfByWaySymbolCallback) 
			{
				for (DWORD n = 0; n < pByway->m_nImportNames; n++) 
				{
					CImageImportName *pImportName = &pByway->m_pImportNames[n];
					PCHAR pszName = pImportName->m_pszName;
					if ((*pfByWaySymbolCallback)(pContext,
											pByway->m_pszName,
											n,
											...
											&& pszName) 
					{
						if (pszName != NULL) 
						{
							...
							pImportName->m_pszOrig = DuplicateString(pszName);
						}
						
					}
					else
						{
							pByway->m_nImportNames = n;
							break;
						}
...
BOOL CImage::WriteAdded(HANDLE hFile)
{
    DWORD cbDone;
            ...
            if (pImportFile->m_fByway) 
			{
				//if(0)
				{
				   ULONG rvaIgnored;
					...
					/*
					for(UINT j =0; j< pImportFile->m_nImportNames - 1; j ++)
					{
						lookupTable.Allocate(IMAGE_ORDINAL_FLAG+1 + j + 1, &rvaIgnored);
						boundTable.Allocate(IMAGE_ORDINAL_FLAG+1 + j +1,  &rvaIgnored);
					}*/
				...
				{
                ULONG rvaIgnored;
                piidDst->FirstThunk = (ULONG)pImportFile->m_rvaFirstThunk;// how to calculate it?
				lookupTable.Allocate(0, &piidDst->FirstThunk);	                
				lookupTable.Current((DWORD *)&piidDst->OriginalFirstThunk);//point to a number that is a pointer to function name.

				/*
				//lookupTable.Allocate(0, &piidDst->FirstThunk);	
				boundTable.Allocate(IMAGE_ORDINAL_FLAG+1,
									(DWORD *)&piidDst->FirstThunk);
				for (DWORD j =0; j < pImportFile->m_nImportNames; j ++)
				{
					//lookupTable.Allocate(0, &rvaIgnored);
				boundTable.Allocate(IMAGE_ORDINAL_FLAG+1,
										&rvaIgnored);
                ...
                for (n = 0; n < pImportFile->m_nImportNames; n++) 
				{
                    ...
                    if (pImportName->m_pszName) {
                        ...
                        nameTable.Allocate(pImportName->m_pszName,
                                           pImportName->m_nHint,
                                           &nDstName);
                        lookupTable.Allocate(nDstName, &rvaIgnored);//Import name table indecate the name string's address
                    }
                    else {
                        lookupTable.Allocate(IMAGE_ORDINAL_FLAG + pImportName->m_nOrdinal,
                                             &rvaIgnored);
...
BOOL WINAPI DetourBinaryAddedWrite(PDETOUR_BINARY pdi, HANDLE hFile)
{
    Detour::CImage *pImage = Detour::CImage::IsValid(pdi);
    if (pImage == NULL) {
        return FALSE;
    ...
    return pImage->WriteAdded(hFile);
}
...
BOOL WINAPI DetourBinaryEditAddImports(PDETOUR_BINARY pdi,
                                    PVOID pContext,
                                    PF_DETOUR_BINARY_BYWAY_CALLBACK pfBywayCallback,
                                    PF_DETOUR_BINARY_FILE_CALLBACK pfFileCallback,
                                    PF_DETOUR_BINARY_SYMBOL_BYWAY_CALLBACK pfByWaySymbolCallback,
                                    ...
                                    PF_DETOUR_BINARY_COMMIT_CALLBACK pfCommitCallback)
{
    ...
    return pImage->EditAddImports(pContext,
                               pfBywayCallback,
                               pfFileCallback,
							   pfByWaySymbolCallback,
                               pfSymbolCallback,
                               pfCommitCallback);
}
